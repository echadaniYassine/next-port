...
export const DecorativeDots = memo(function DecorativeDots({
  variant = "default",
  position = "top-left",
  colors = "blue-purple",
  className = "",
  ...
  const getColors = () => {
    switch (colors) {
      case "pink-red":
        return { primary: "from-pink-400 to-red-600", secondary: "from-rose-400 to-pink-600" }
      case "emerald-teal":
        return { primary: "from-emerald-400 to-teal-600", secondary: "from-teal-400 to-cyan-600" }
      case "orange-yellow":
        return { primary: "from-orange-400 to-yellow-600", secondary: "from-amber-400 to-orange-600" }
      case "mixed":
        return { primary: "from-blue-400 to-purple-600", secondary: "from-pink-400 to-red-600" }
      default:
        return { primary: "from-blue-400 to-purple-600", secondary: "from-purple-400 to-indigo-600" }
    }
  }
  ...
  const renderDots = (pos: string) => {
    ...
    return (
      <>
        <div className={`absolute ${pos_config.primary} ${sizes.primary} bg-gradient-to-r ${colorScheme.primary} rounded-full opacity-20 animate-pulse`} />
        <div className={`absolute ${pos_config.secondary} ${sizes.secondary} bg-gradient-to-r ${colorScheme.secondary} rounded-full opacity-20 animate-pulse animation-delay-1000`} />
      </>
    )
  }
...
// --- Component 2: FloatingParticles (FIXED for Hydration) ---
...
export const FloatingParticles = memo(function FloatingParticles({
  ...
  useEffect(() => {
    ...
    const styles = Array.from({ length: count }).map(() => ({
      ...
      animationDuration: `${getSpeedValue() + Math.random() * 2}s`,
    }));
    setParticleStyles(styles);
  }, [count, speed]); // Re-generate if props change

  const sizeClass = size === "small" ? "w-1 h-1" : size === "large" ? "w-3 h-3" : "w-2 h-2";
  ...
  return (
    <div className={`absolute inset-0 overflow-hidden pointer-events-none ${className}`}>
      {particleStyles.map((style, i) => (
        <div
          key={i}
          className={`absolute ${sizeClass} bg-gradient-to-r ${colors[i % colors.length]} rounded-full opacity-20 animate-float`}
          style={style}
        />
      ))}
    </div>
...
export const GradientOrbs = memo(function GradientOrbs({ variant = "section", className = "" }: GradientOrbsProps) {
  const getOrbConfig = () => {
    switch (variant) {
      case "hero":
        return {
          size: "w-96 h-96", blur: "blur-3xl", orbs: [
      ...
      case "card":
        return {
          size: "w-32 h-32", blur: "blur-2xl", orbs: [
      ...
      default:
        return {
          size: "w-64 h-64", blur: "blur-2xl", orbs: [
